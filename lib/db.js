...
class Database {
  ...
  sync(filename) {
    const dbScript = new pg.QueryFile(filename, {minify: true});
    return this.db.none(syncSql());
  }
  insertRegistration(token, auth, user) {
    return this.db.none('insert into records (token, oauth, username) values ($1, $2, $3) ' + 
  ...
  removeExpiredRegistration(token) {
    return this.db.none('delete from records where token = $1', [token]);
  }
  isRegistered(token, oauth) {
    return this.db.oneOrNone('select 1 from records where token = $1 and oauth = $2', [token, oauth], r => !!r);
  }
