...
class Database {
  ...
  insertRegistration(token, auth, user) {
    return this.db
      .none(
        'insert into records (token, oauth, username) values ($1, $2, $3) ' +
          'on conflict (token, username) do update set oauth = excluded.oauth, ' +
          "updated_at = (now() at time zone 'utc')",
        [token, auth, user],
      )
      .then(() => this.db.none('update records set oauth = $1 where username = $2', [auth, user]));
  }
  ...
  updateUpdatedAt(oauth) {
    return this.db.none(
      "update records set updated_at = (now() at time zone 'utc') where oauth = $1",
      [oauth],
    );
  }
}
